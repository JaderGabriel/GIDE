...
> **Duas filas no admin (não confundir):** `/admin/gestor-access-events` mostra `gestor_access_event_deliveries` — uma linha por POST do webhook Gestor/catraca. O POST ao iEducar (catraca-frequência) **só é tentado** se existir integração `ieducar` com **`enabled=true`** e o motor de presença devolver **`action=mark_presence`** com `cod_aluno` válido. Por padrão, o motor considera presença **permitida** (decide pelas janelas) quando `action.mark_presence` não vem no payload; ele **só bloqueia** presença quando `action.mark_presence=false` é declarado. Já `/admin/frequencia-ieducar` lista **`ieducar_frequencia_registro_deliveries`** (registros enfileirados por outro fluxo, ex. comando Artisan ou job); uma pode mostrar sucessos enquanto a outra, para o mesmo dia, mostra “sem POST” se o evento de acesso não cumpriu as condições acima ou foi processado quando o iEducar estava desligado.
