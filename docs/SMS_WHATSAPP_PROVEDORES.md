...
- Na página de detalhe da entrega (`admin/gestor-access-events/{id}`), o painel **SMS — reenvio manual** aparece **após** o cartão **Horário do evento** (quando esse cartão existe). Os quatro JSON (payload bruto, motor, pedido e resposta iEducar) ficam numa **grelha 2×2** em ecrãs largos. O bloco **«SMS enviados neste evento»** fica **no final** da página, após esses JSON e o histórico de reprocessamentos. A **cronologia** unificada vem de `send_log` (útil para distinguir envio automático de reenvios manuais). Registos antigos podem não ter linha de tempo até haver um envio concluído com código atual.
